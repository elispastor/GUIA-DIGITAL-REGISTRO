...
// Cloudflare Worker — recibe formulario y genera tarjeta
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    // 5. Guardar en colección "afiliados"
    const afiliado = {
      nombre,
      negocio,
      telefono,
    ...
    // 6. Generar HTML de la tarjeta
    const htmlTarjeta = generarHTMLTarjeta(afiliado);

    // 7. Guardar en colección "tarjetas"
    const tarjeta = {
    ...
    await db.collection("tarjetas").insertOne(tarjeta);

    // 8. Agregar a colección "negocios" (para la guía)
    const negocioGuia = {
      afiliado_id: afiliadoId,
      nombre,
      negocio,
    ...
    await db.collection("negocios").insertOne(negocioGuia);

    await client.close();

    // 9. Devolver respuesta con la URL de la tarjeta
